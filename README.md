...
OS_Agent 是一个基于大语言模型的 Linux 运维助手。用户在终端输入自然语言，系统生成结构化执行计划、进行风险判断、执行命令，并把审计结果保存到本地。

## 项目来源

- 本项目是在开源项目 LinuxAgent 的基础上进行的二次开发。
- 原项目作者与仓库：`Eilen6316` / `https://github.com/Eilen6316/LinuxAgent`
- 当前仓库在保留原有能力思路的基础上，面向 操作系统智能代理 方向做了交互和工程化调整。
...
脚本会执行 Python 语法编译和启动前配置校验，确保核心代码可导入、配置路径可用且密钥输出保持脱敏。

如需进行赛题能力验收，可参考 [docs/hackathon/self-test.md](./docs/hackathon/self-test.md)，通过真实 CLI 交互和 `~/.os_agent/tasks` 下的审计文件检查以下能力：
...
3. 运行 `bash scripts/self_check.sh`，确保语法编译和配置校验通过。
